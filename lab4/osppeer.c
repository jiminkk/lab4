...
static size_t read_tracker_response(task_t *t)
{
	...
	while (1) {
		...
		//NOTE do some checking here to see arguments or something 
...
static void register_files(task_t *tracker_task, const char *myalias)
{
	...
	while ((ent = readdir(dir)) != NULL) {
		...
		messagepos = read_tracker_response(tracker_task);
		//Potential weakness from read_tracker_reponse does it buffer overflow?
		if (tracker_task->buf[messagepos] != '2')
...
//	the requested file.
//	user can specify any file in the directory which is not supposed to happen MUST fix 
//
